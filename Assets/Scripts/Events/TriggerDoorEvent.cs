...
public class TriggerDoorEvent : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (doorTransform != null)
        {
            ...
            doorTransform.localRotation = Quaternion.Euler(0f, -450f, 0f); // Gira a -90 grados en Y
            Debug.Log("Door rotated to: " + doorTransform.localRotation.eulerAngles); // Muestra la rotación en la consola
        }
        else
        {
            Debug.LogError("doorTransform no está asignado.");
        }
        startDoorRot = doorTransform.localRotation;        
    }
